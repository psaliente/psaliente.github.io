...
function ProjectListItem(projectItem: ProjectListItemPropType) {
  ...
  const [doneWait, setDoneWait] = useState(!!wait === false);
  ...
  return (
    <Activity mode={doneWait ? 'visible' : 'hidden'}>
      <Link to="/project" onClick={setCurrentProjectItem}>
        <Card className="w-xs sm:w-sm h-(--project-card-height) fade-in gap-4 px-7 pb-5 pt-8 bg-white dark:bg-gray-800 justify-center transition delay-300 duration-500 hover:scale-[1.02]">
